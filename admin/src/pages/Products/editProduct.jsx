...
const EditProduct = () => {
    ...
    useEffect(() => {
        ...
        //Assuming there is an API for productColor; if not, use default data
        const defaultColors = [
            { productColor: "Red" },
            { productColor: "Xanh" },
            { productColor: "Vàng" },
    ...
    const editProduct = (e) => {
        ...
        if (formFields.price === null || formFields.price <= 0 || !/^\d+(\.\d+)?$/.test(formFields.price)) {
            context.setAlterBox({
                open: true,
                error: true,
                message: "Please enter the product price as a number greater than 0, without special characters or letters"
            });
            setLoading(false);
            return false;
        }
        ...
        if (formFields.oldPrice === null || formFields.oldPrice <= 0 || !/^\d+(\.\d+)?$/.test(formFields.price)) {
            context.setAlterBox({
                open: true,
                error: true,
                message: "Please enter the original product price as a number greater than 0, without special characters or letters"
            });
            setLoading(false);
            return false;
        }
