...
router.post(`/signup`, async (req, res) => {
  ...
  try {
    ...
    if (existingUserByPhone) {
      return res.status(400).json({
        error: true,
        message: "This phone number is already in use; you can log in using this phone number"
      });
    }
...
router.post(`/login`, async (req, res) => {
  ...
  try {
    ...
    if (!emailOrPhone || !emailOrPhone.trim()) {
      return res.status(400).json({
        error: true,
        message: "Email or phone number cannot be left blank"
      });
    }
...
router.post(`/google-auth`, async (req, res) => {
  try {
    ...
    if (user && user.password) {
      return res.status(400).json({
        error: true,
        message: "Email is registered via another method. Please log in with email & password.",
      });
    }
...
router.post(`/forgot-password`, async (req, res) => {
  try {
    ...
    if (user.email) {
      ...
      return res.status(400).json({
        error: true,
        message: "This account has no email to send the password reset link",
      });
    }
...
//Only Admin or the user themselves can view information
...
//Only Admin or the user themselves can update information
...
//Only Admin or the user themselves can add the address
...
//Only Admin or the user themselves can update the address
...
//Only Admin or the user themselves can delete the address
